...
c
      ...
      SUBROUTINE SOLVE (U)
C  הPAךBEP PEûEHיס CיCTEMש HEליHEךHשX ץPABHEHיך
C          METOהOM HראTOHA.
C
C  CM.:     הEHHיC הצ.,Mל.,ûHAגEלר P.
C       ‏יCלEHHשE METOהש גEתץCלOBHOך MיHיMיתAדיי
C       י PEûEHיס HEליHEךHשX ץPABHEHיך:נEP. C AHחל-
C       M.:MיP,1988ח.
C
C         נAPAMETPש י נEPEMEHHשE:
C  נOCTABיTר!
C
...
C     IRANG PABEH PAתMEPHOCTי תAהA‏י.
      INTEGER         IRANG
...
C  P-P CBסתAH C IRANG י PABEH נOלOBיHE IRANG.
...
C      ENDDO  
   ...
   33 FORMAT(2X,'ZNEWSOL (קטןה) :  N=',I4)
      IF(KPRSOL.GE.2) WRITE(6, 2) (U(I),I=1,N)


C  Bש‏יCלEHיE MACHEPS
      CALL MACHEP(EPSIM)
C  תAהAHיE י נPOBEPKA BXOהHOך יHזOPMAדיי
...
C  Bש‏יCליM BEKTOP-זץHKדיא
      CALL NEF(IRANG2,U,FN,SF,FNOR,FLAG,FLGFFT)
C    FLAG- נPיתHAK OûיגKי B נ/נ MOהEלי H.ü.

C  נPOBEPיM, MOצET ץצE ECTר PEûEHיE
      CALL STOP0(N,FN,U,SF,SX,TERMCD,KMAXDU)
      IF(TERMCD.EQ.1)RETURN
C  Bש‏יCליM סKOגיAH
      CALL NEJAC(IRANG2,U,DFDX,FLAG,FLGFFT)

C  Bש‏יCליM חPAהיEHT נOלOBיHש
C   KBAהPATA HOPMש HEBסתKי נO נEPEMEHHOך
      CALL GRADIE(IRANG,N,DFDX,FN,SF,GR)
C F <- FN
      DO 20 I=1,N
   20 F(I)=FN(I)
C סKOגיAH CEך‏AC AHAליTי‏ECKיך
...
C  PEûEHיE AזזיHOך MOהEלי
 ...
 5100 FORMAT(2X,'ZNEWSOL (40)    : DFDX='/(3X,6(E12.6)))
C 5100 FORMAT(2X,'ZNEWSOL (40)    : DFDX=',3X,E12.6) 
C   40 CONTINUE

      CALL LINEQ1(DFDX,IRANG,N,IRANG1,1,INDEX,NER,DET,ARGD)
C  INTEGER INDEX(IRANG),COMPLEX DET,REAL ARGD,INTEGER NER  ץצE
C  Oג'סBלEHש. NER=1-BCE XOPOûO,NER=0-OגHAPץצEH HץלEBOך CTOלגEד.
      IF(KPRSOL.GE.3) WRITE(6, 5200) (DFDX(I,N1),I=1,N)
      IF(KPRSOL.GE.3) PRINT    5200, (DFDX(I,N1),I=1,N)
 5200 FORMAT(2X,'ZNEWSOL (LINEQ1): DFDX='/(3X,6(E12.6))) 

      DO 50 I=1,N
   50 Y(I)=DFDX(I,N1)
      IF(KPRSOL.GE.3) WRITE(6, 4000) (Y(I),I=1,N)
      IF(KPRSOL.GE.3) PRINT    4000, (Y(I),I=1,N)
 4000 FORMAT(2X,'ZNEWSOL (50)    : Y='/(3X,6(E12.6))) 

C  OTCE‏KA MHיMשX ‏ACTEך נEPEMEHHשX HA HץלEBOך ‏ACTOTE
      CALL OTCHM(Y,K3)
      IF(KPRSOL.GE.3) WRITE(6, 4100) (Y(I),I=1,N)
      IF(KPRSOL.GE.3) PRINT    4100, (Y(I),I=1,N)
 4100 FORMAT(2X,'ZNEWSOL (OTCHM) : Y='/(3X,6(E12.6))) 

C   Bש‏יCלEHיE ץחלA MEצהץ חPAה. י נOנPABKOך. OדEHKA OגץCל. סKOגיAHA .
C   L2  HOPMA חPAהיEHTA  י נOנPABKי
      SLOPE=0.D0
      GRNOR=0.D0
      YNOR=0.D0
      DO 2001 II=1,N
...
C   .1. AנPיOPHAס
 ...
 4200 FORMAT(2X,'ZNEWSOL (SUPRIM): Y='/(3X,6(E12.6))) 
C
C     נOנPABKA
      DO 60 I=1,N
   60 UN(I)=U(I)-Y(I)
...
C .2. ליHEךHשך נOיCK.
 ...
 4300 FORMAT(2X,'ZNEWSOL (LSERCH): Y='/(3X,6(E12.6))) 

C  ECלי HE נPOBOהילACר חלOגAליתAדיס,TO HץצHO Bש‏יCליTר UN,FN,FNORN
...
C  תABEPûAEM יTEPAדיא...
C    סKOגיAH:
      CALL NEJAC(IRANG2,UN,DFDX,FLAG,FLGFFT)
C    חPAהיEHT:
      CALL GRADIE(IRANG,N,DFDX,FN,SF,GR)
C  נPBEPKA ץCלOBיך OCTAHOBA
 ...
 4400 FORMAT(2X,'ZNEWSOL (STOP): Y='/(3X,6(E12.6))) 
...
C  U <- UN , F <- FN , FNOR <- FNORN
  ...
  110 F(I)=FN(I)
      ...
      GOTO 1000
C___K_O_H_E_ד__________________________________________________________
1100  CONTINUE
